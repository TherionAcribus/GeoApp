"""Plugin POSTNET Barcode pour MysterAI.

Ce plugin encode/décode le code-barres POSTNET (Postal Numeric Encoding
Technique) utilisé par l'USPS pour représenter un code ZIP sous forme de
barres hautes/basses. Il supporte :
- L'encodage d'une suite de chiffres en ZIP-5 / ZIP+4 / ZIP+4+2 / format libre
...
https://fr.wikipedia.org/wiki/Postnet
"""
...
POSTNET_DECODING = {v: k for k, v in POSTNET_ENCODING.items()}


class PostnetBarcodePlugin:
    """Plugin d'encodage/décodage du code-barres POSTNET.

    Args:
        inputs (dict):
            - text (str): chiffres à encoder, ou barres à décoder
            - mode (str): 'encode' ou 'decode'
            - format (str, optionnel): 'auto'|'zip5'|'zip9'|'zip11'|'free' (encode)
    ...
    def _execute_encode(
        ...
        encoded = self._encode_to_postnet(digits, format_type, checksum_mode, frame_bars, visual_format)

        if encoded["format"] == "free":
            format_name = f"libre ({len(encoded['digits'])} chiffres, {len(encoded['binary'])} barres)"
        elif len(encoded["binary"]) == 32:
            format_name = "ZIP-5 (32 barres)"
        elif len(encoded["binary"]) == 52:
            format_name = "ZIP+4 (52 barres)"
        elif len(encoded["binary"]) == 62:
    ...
    def _encode_to_postnet(
        ...
        if target_format == "auto":
            target_format = {5: "zip5", 9: "zip9", 11: "zip11"}.get(len(clean_digits), "free")
        ...
        add_checksum = checksum_mode in {"required", "optional"} or (
            checksum_mode == "auto" and target_format in {"zip5", "zip9", "zip11"}
        )
        ...
        add_frame_bars = frame_bars == "always" or (
            frame_bars == "auto" and target_format in {"zip5", "zip9", "zip11"}
        )
        ...
        for digit in full_code:
            if digit not in POSTNET_ENCODING:
                raise ValueError(f"Caractère non supporté: {digit}")
            postnet_code += POSTNET_ENCODING[digit]
        if add_frame_bars:
            postnet_code += "1"

        pipe_dot_format = self._format_to_visual(postnet_code, "pipe_dot")
        pipe_down_format = self._format_to_visual(postnet_code, "pipe_down")
        display_format = {
            "binary": postnet_code,
    ...
    def _execute_decode(
        ...
        patterns = self._detect_postnet_pattern(text)
        if not patterns:
        ...
        if not results:
            return self._error_response("Aucun code POSTNET valide détecté", start_time)
    ...
    def _execute_bruteforce_decode(self, text: str, start_time: float) -> Dict[str, Any]:
        ...
        for i, variation in enumerate(variations, 1):
            results.append(
                {
                    "id": f"result_{i}",
                    "text_output": self._format_zip(variation["text_output"]),
    ...
    @staticmethod
    def _format_zip(zip_code: str) -> str:
        if len(zip_code) == 9:
            return f"{zip_code[:5]}-{zip_code[5:]}"
        if len(zip_code) == 11:
            return f"{zip_code[:5]}-{zip_code[5:9]}-{zip_code[9:]}"
        return zip_code
    ...
    def _decode_from_postnet(self, barcode: str, flexible: bool = False) -> Dict[str, Any]:
        ...
        for i in range(0, len(data_portion), 5):
            ...
            if pattern.count("1") != 2 or pattern not in POSTNET_DECODING:
                if not flexible:
                ...
                continue
            digits += POSTNET_DECODING[pattern]
    ...
    @staticmethod
    def _detect_postnet_pattern(text: str) -> List[Dict[str, Any]]:
        ...
        for flexible in (True, False):
            for assume_frame_bars in (True, False):
                try:
                    ...
                    result = self._decode_from_postnet(test_text, flexible)
