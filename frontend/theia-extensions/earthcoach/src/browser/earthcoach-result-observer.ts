...
import { EarthCoachResultCaptureService, extractEarthCoachResultBlock } from './earthcoach-result-capture';
...
export class EarthCoachResultObserver implements GeoAppChatResponseObserver {
    ...
    async handleChatResponse(event: GeoAppChatResponseEvent): Promise<void> {
        ...
        if (!payload) {
            try {
                await this.capture.attachLatestMarkdown(event.text || '', event.sessionId);
            } catch (error) {
        ...
        try {
            await this.capture.capture(payload, { markdown: event.text, sessionId: event.sessionId });
        } catch (error) {
