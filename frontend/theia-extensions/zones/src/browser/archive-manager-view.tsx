...
export interface ArchiveManagerViewProps {
    isLoading: boolean;
...
const ArchiveHistoryCard: React.FC<ArchiveHistoryCardProps> = ({
    ...
    const selectedReplayStepId = replayStepSelections[historyEntryKey]
    ...
    return (
        <div style={{ padding: 16, display: 'grid', gap: 16, maxWidth: 1180 }}>
            <h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                >
                    {props.isLoading ? '...' : 'Rafraichir'}
                </button>
            </h2>

            <div style={{ background: 'var(--theia-editor-background)', border: '1px solid var(--theia-panel-border)', borderRadius: 6, padding: 16 }}>
                ...
                {props.isLoading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
                        {[
                            { label: 'Total archivees', value: props.stats.total_archived, color: '#60a5fa' },
                            { label: 'Resolues', value: props.stats.solved, color: '#10b981' },
                            { label: 'En cours', value: props.stats.in_progress, color: '#f59e0b' },
                            { label: 'Trouvees', value: props.stats.found, color: '#a78bfa' },
            ...
            <div
                style={{
                    ...
                    border: `1px solid ${props.autoSync ? 'var(--theia-panel-border)' : '#f59e0b'}`,
                    borderRadius: 6,
                    padding: 16,
                }}
            >
                ...
                {!props.autoSync ? (
                    ...
                    >
                        <strong>Archivage automatique desactive.</strong> Les donnees de resolution ne sont plus
                        sauvegardees automatiquement. Le snapshot avant suppression reste actif.
                    </div>
                ) : undefined}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                        <div style={{ fontWeight: 'bold', fontSize: 13 }}>
                            {props.autoSync ? 'Active (recommande)' : 'Desactive (non recommande)'}
                        </div>
                        <div style={{ fontSize: 11, opacity: 0.7, marginTop: 4, maxWidth: 480 }}>
                            Synchronise automatiquement l'archive lors des changements d'etat
                            (statut, coordonnees, notes, waypoints). Le snapshot avant suppression
                            reste toujours actif.
                        </div>
                    </div>
                    <button
                        className={`theia-button ${props.autoSync ? 'secondary' : ''}`}
                        onClick={props.onToggleAutoSync}
                        disabled={props.isSaving}
                        style={{ fontSize: 12, padding: '6px 14px', whiteSpace: 'nowrap' }}
                    >
                        {props.isSaving ? '...' : props.autoSync ? 'Desactiver' : 'Activer'}
                    </button>
                </div>
            </div>
                    ...
                    <div style={{ border: '1px solid var(--theia-panel-border)', borderRadius: 6, overflow: 'hidden', minHeight: 280 }}>
                        <div style={{ display: 'grid', gap: 1, background: 'var(--theia-panel-border)' }}>
                            {props.archives.length === 0 ? (
                                <div style={{ background: 'var(--theia-editor-background)', padding: 12, fontSize: 12, opacity: 0.7 }}>
                                    {props.isLoadingArchives ? 'Chargement des archives...' : 'Aucune archive trouvee pour ce filtre.'}
                                </div>
                            ) : props.archives.map(entry => {
                                const isSelected = entry.gc_code === props.selectedArchiveGcCode;
                                const historyCount = entry.resolution_diagnostics?.history_state?.length
                                    || (entry.resolution_diagnostics?.resume_state ? 1 : 0);
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', borderTop: '1px solid var(--theia-panel-border)', fontSize: 11 }}>
                            <button className='theia-button secondary' disabled={props.archivesPage <= 1 || props.isLoadingArchives} onClick={props.onPreviousArchivesPage}>
                                Precedent
                            </button>
                            <span>Page {props.archivesPage}/{props.archivePages}</span>
                            <button className='theia-button secondary' disabled={props.archivesPage >= props.archivePages || props.isLoadingArchives} onClick={props.onNextArchivesPage}>
                                Suivant
                            </button>
                        </div>
                    </div>
                                ...
                                {props.selectedArchive.resolution_diagnostics?.current_text ? (
                                    ...
                                    >
                                        ...
                                        <div style={{ fontSize: 12 }}>
                                            {truncateArchiveText(props.selectedArchive.resolution_diagnostics.current_text, 260)}
                                        </div>
                                    </div>
                                ) : undefined}
            ...
            <div style={{ background: 'var(--theia-editor-background)', border: '1px solid #ef444466', borderRadius: 6, padding: 16 }}>
