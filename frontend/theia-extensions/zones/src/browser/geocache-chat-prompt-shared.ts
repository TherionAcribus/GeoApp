...
export function buildGeocacheChatPrompt(data: GeocachePromptData): string {
    ...
    return [
        ...
        '- Apres resolve_geocache_workflow, enchaine IMMEDIATEMENT avec run_geocache_workflow_step(geocache_id, target_step_id) pour executer l etape suivante : inspect-images, describe-images, execute-direct-plugin, execute-metasolver, search-answers, calculate-final-coordinates ou validate-with-checker. Ne demande pas a l utilisateur de le faire a ta place : c est TOI qui appelles le tool.',
        ...
        '- Si le listing donne deux points de reference et deux distances/rayons (ex: "x km from P1" et "y km from P2"), appelle coordinate_intersection avec text et strict="smooth" avant toute estimation manuelle.',
        ...
        '',
        'Images / OCR :',
        '- Si resolve_geocache_workflow choisit image_puzzle, appelle IMMEDIATEMENT run_geocache_workflow_step(geocache_id, "inspect-images") dans la meme reponse, sans attendre de confirmation. Ne decris pas ce que tu prevois de faire : FAIS-LE.',
        '- Si inspect-images remonte un selected_fragment ou une recommendation metasolver (images avec texte/QR/code), repars de ces sorties plutot que du listing brut.',
        '- Si inspect-images ne remonte PAS de selected_fragment (images source: image_vision_description ou aucun fragment), c est que les images sont des illustrations visuelles sans texte. Dans ce cas, appelle immediatement run_geocache_workflow_step(geocache_id, "describe-images") pour obtenir une description semantique de chaque image par le modele vision.',
        '- La sortie de describe-images contient des descriptions textuelles par image (ex: "Cette image illustre Boucle d Or et les Trois Ours"). Utilise ces descriptions pour identifier le theme (conte, legende, film), puis applique la logique metier du listing (compter les mots du titre, etc.).',
        '- Les descriptions image (source: image_vision_description) sont des indices semantiques, pas des codes secrets. Ne les passe pas dans metasolver.',
        '- Tu peux aussi passer describe_context dans run_geocache_workflow_step pour specialiser le prompt vision (ex: describe_context: "contes pour enfants").',
        '',
